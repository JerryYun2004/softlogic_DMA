...
`define YOLO_IMAGE_MEM "yolo_image_1x1.mem"
`endif

`ifndef YOLO_WEIGHTS_MEM
`define YOLO_WEIGHTS_MEM "yolo_weights_1x1.mem"
`endif

`ifndef YOLO_GOLDEN_MEM
`define YOLO_GOLDEN_MEM "golden_conv1.mem"
`endif

// Combined YOLO 1x1 regression:
//   * The CPU-facing dma_a AXI4-Lite wrapper receives every DMA command.
//   * conv1x1_hwc_channel_dma_agu loads the activation SRAM banks.
//   * The same DMA loads every 8x8 weight slice into the NPU.
//   * All 32x32x16 results are compared with golden_conv1.mem.
//
...
// and applies one weight-shift cycle.  This verifies the complete
// AXI-Lite-wrapper/AGU/NPU integration, but it is not the future physical SRAM
// datapath.
module tb_npu_yolo_conv_3x3_dma_integrated;
    ...
    localparam int WEIGHT_BYTES       = INPUT_CHANNELS * OUTPUT_CHANNELS;
    localparam int OUTPUT_WORDS       =
        IMAGE_HEIGHT * IMAGE_WIDTH * OUTPUT_CHANNELS;

    localparam int GROUPS_PER_ACT_LOAD     = 16 * 16;
    localparam int EXPECTED_DMA_LOADS      = 2 * 2 * 2 * 2;
    localparam int EXPECTED_ACT_GROUPS     =
        EXPECTED_DMA_LOADS * GROUPS_PER_ACT_LOAD;
    localparam int EXPECTED_WEIGHT_LOADS   = 2 * 2 * 2 * 2;
    localparam int EXPECTED_WEIGHT_GROUPS  =
        EXPECTED_WEIGHT_LOADS * 8;
    localparam int EXPECTED_WEIGHT_BYTES   =
        EXPECTED_WEIGHT_GROUPS * 8;

    localparam logic [1:0] AXI_RESP_OKAY = 2'b00;

    // Non-zero bases verify that the adapter interprets DMA byte addresses,
    // rather than accidentally treating them as raw array indices.
    ...
    // ---------------------------------------------------------------------
    // During DMA transfers the adapter owns the activation write ports or the
    // NPU weight-shift inputs.  During compute, act_compute_addr owns the
                ...
                for (int r = 0; r < ARRAY_HEIGHT; r++) begin
                    if (map_bank_mask[r] &&
                        ...
                        dma_weight_source_offset =
                            ...
                            r*map_source_stride;
                        weight_shift_in[r] =
                            raw_weights_1x1_flat[dma_weight_source_offset];
                    end else begin
                        weight_shift_in[r] = 'x;
                    end
                end
                    ...
                    if (map_valid && map_bank_mask[r]) begin
                        ...
                        if (map_zero_fill) begin
                            ...
                            ext_act_sram_wdata[r] =
                                raw_image_1x1_flat[dma_source_offset];
                        end else begin
    ...
    // Monitor every command accepted by the SRAM adapter.
    always @(posedge clk_i) begin
        if (!rst_n) begin
            dma_total_groups        <= 0;
                ...
                if (map_is_weight) begin
                    ...
                    if (map_zero_fill) begin
                        dma_total_zero_groups <=
                            dma_total_zero_groups + 1;
                        $display("ERROR: 1x1 activation DMA requested zero fill");
                        errors = errors + 1;
                    end
                    if (!((map_source_addr >= IMAGE_BASE) &&
                          (map_source_addr + 7*map_source_stride <
                           IMAGE_BASE + IMAGE_BYTES))) begin
    ...
    // AXI4-Lite driver
    // ---------------------------------------------------------------------
    // AW and W are launched together but tracked independently, so this task
    // remains valid for the wrapper's decoupled AXI-Lite write channels.
    task automatic axil_write(input logic [9:0]  addr,
        ...
        begin
            ...
            // axil_bready is held high; allow the response handshake to
            // complete before another transaction begins.
            @(posedge clk_i);
            #1;
        end
    endtask
    ...
    // ---------------------------------------------------------------------
    // DMA commands issued through the AXI4-Lite wrapper
    // ---------------------------------------------------------------------
    task automatic dma_load_1x1_activations(input int tile_y,
        ...
        begin
            ...
            if (dma_status[3]) begin
                $display("ERROR: DMA wrapper error after activation load ty=%0d tx=%0d cin=%0d",
                         tile_y, tile_x, cin_block);
                errors = errors + 1;
            end
            if (dma_groups_this_load != GROUPS_PER_ACT_LOAD) begin
                $display("ERROR: activation DMA ty=%0d tx=%0d cin=%0d emitted %0d groups, expected %0d",
                         tile_y, tile_x, cin_block,
                         dma_groups_this_load, GROUPS_PER_ACT_LOAD);
                errors = errors + 1;
            end
            if (dma_last_this_load != 1) begin
                $display("ERROR: activation DMA ty=%0d tx=%0d cin=%0d emitted map_last %0d times",
                         tile_y, tile_x, cin_block, dma_last_this_load);
                errors = errors + 1;
            end
            if (dma_swap_this_load != 0) begin
                $display("ERROR: activation DMA emitted %0d weight swaps",
                         dma_swap_this_load);
                errors = errors + 1;
            end
        end
    ...
    task automatic dma_load_weights_slice(input int cin_block,
        ...
        begin
            ...
            if (dma_status[3]) begin
                $display("ERROR: DMA wrapper error after weight load cin=%0d cout=%0d",
                         cin_block, cout_block);
                errors = errors + 1;
            end
            if (dma_groups_this_load != 8) begin
                $display("ERROR: weight DMA cin=%0d cout=%0d emitted %0d groups, expected 8",
                         cin_block, cout_block, dma_groups_this_load);
                errors = errors + 1;
            end
            if (dma_last_this_load != 1) begin
                $display("ERROR: weight DMA cin=%0d cout=%0d emitted map_last %0d times",
                         cin_block, cout_block, dma_last_this_load);
                errors = errors + 1;
            end
            if (dma_swap_this_load != 1) begin
                $display("ERROR: weight DMA cin=%0d cout=%0d emitted swap %0d times",
    ...
    initial begin : main_test
        ...
        $readmemh(image_mem_path, raw_image_1x1_flat);
        $readmemh(weights_mem_path, raw_weights_1x1_flat);
        $readmemh(golden_mem_path, golden_conv1_flat);

        repeat (5) @(posedge clk_i);
        @(negedge clk_i);
        rst_n = 1'b1;

        // Static identity bank-to-PE-row selection.
        for (int r = 0; r < ARRAY_HEIGHT; r++)
            crossbar_sel[r] = r[BANK_SEL_WIDTH-1:0];

        $display("===================================================================");
        $display(" YOLO 1x1 INTEGRATED TEST: AXI DMA + ACTIVATIONS + WEIGHTS + NPU");
        $display("===================================================================");
            ...
            for (int tx = 0; tx < 2; tx++) begin
                ...
                for (int cout_block = 0; cout_block < 2; cout_block++) begin
                    ...
                    for (int cin_block = 0; cin_block < 2; cin_block++) begin
                        ...
                        dma_load_weights_slice(cin_block, cout_block);
                        run_conv1x1_pass(swap_mode, hold_zero);
                        pass_idx = pass_idx + 1;
                    end

                    // Two input-channel passes leave the final result in banks
                    // 0..7, matching the 3x3 regression's alternating scheme.
        ...
        if (dma_load_count != EXPECTED_DMA_LOADS) begin
            $display("ERROR: activation DMA loads=%0d expected=%0d",
                     dma_load_count, EXPECTED_DMA_LOADS);
            errors = errors + 1;
        end
        if (dma_total_groups != EXPECTED_ACT_GROUPS) begin
            $display("ERROR: activation DMA groups=%0d expected=%0d",
                     dma_total_groups, EXPECTED_ACT_GROUPS);
            errors = errors + 1;
        end
        if (dma_total_zero_groups != 0) begin
            $display("ERROR: activation DMA zero groups=%0d expected=0",
                     dma_total_zero_groups);
            errors = errors + 1;
        end
        if (weight_load_count != EXPECTED_WEIGHT_LOADS) begin
            $display("ERROR: weight DMA loads=%0d expected=%0d",
                     weight_load_count, EXPECTED_WEIGHT_LOADS);
            errors = errors + 1;
        end
        if (dma_total_weight_groups != EXPECTED_WEIGHT_GROUPS) begin
            $display("ERROR: weight DMA groups=%0d expected=%0d",
                     dma_total_weight_groups, EXPECTED_WEIGHT_GROUPS);
            errors = errors + 1;
        end
        if (dma_total_weight_bytes != EXPECTED_WEIGHT_BYTES) begin
            $display("ERROR: weight DMA bytes=%0d expected=%0d",
                     dma_total_weight_bytes, EXPECTED_WEIGHT_BYTES);
            errors = errors + 1;
        end
        if (dma_total_weight_swaps != EXPECTED_WEIGHT_LOADS) begin
            $display("ERROR: weight swaps=%0d expected=%0d",
        ...
        if (axi_write_count !=
            2 * (EXPECTED_DMA_LOADS + EXPECTED_WEIGHT_LOADS)) begin
            $display("ERROR: AXI writes=%0d expected=%0d",
                     axi_write_count,
                     2 * (EXPECTED_DMA_LOADS + EXPECTED_WEIGHT_LOADS));
            errors = errors + 1;
        end

        $display("-------------------------------------------------------------------");
        $display("AXI-Lite writes             : %0d", axi_write_count);
        ...
        $display("DMA weight groups           : %0d",
                 dma_total_weight_groups);
        $display("DMA weight bytes            : %0d",
                 dma_total_weight_bytes);
        $display("DMA weight swaps            : %0d",
        ...
        if (errors == 0) begin
            $display("PASS: AXI-driven RTL DMA activations and weights produced all 16,384 golden YOLO 1x1 outputs");
            $finish;
        end else begin
            $fatal(1,
                   "FAIL: integrated 1x1 DMA/NPU regression found %0d errors",
                   errors);
        end
    end
    ...
    initial begin : optional_waveform
        if ($value$plusargs("VCD=%s", vcd_path)) begin
            $dumpfile(vcd_path);
            $dumpvars(0, tb_npu_yolo_conv_1x1_dma_integrated);
        end
    end

    initial begin : timeout_watchdog
        repeat (100000) @(posedge clk_i);
        $fatal(1, "FAIL: integrated 1x1 DMA/NPU simulation timeout");
    end
