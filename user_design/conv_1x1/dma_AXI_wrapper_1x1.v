...
// CPU-facing AXI4-Lite wrapper for conv1x1_hwc_channel_dma_agu.
//
// The AXI-facing and map_* port lists are intentionally unchanged from the
// 3x3 DMA wrapper, so top_wrapper.v and the downstream SRAM/weight adapter can
// be reused without interface changes.
//
...
//                              r * map_source_stride
//   activation destination   = map_buffer_addr in each selected bank
//   weight shift index       = map_buffer_addr[2:0]
//
// Register map:
//   0x00 W: source-base byte address and start selected operation
...
//          [1] tile_y
//          [2] cin_block
//          [3] load_weight (0: 16x16 activation tile, 1: 8x8 weight slice)
//          [4] cout_block  (weight mode)
//          [8:5] reserved, read as zero
//
// Software writes 0x08 before each write to 0x00. In activation mode, the
// address at 0x00 is image_base. In weight mode, it is weight_base.
module dma_a (
    input  wire        clk,
    input  wire        rstn,
    ...
    // AXI-Lite write-address and write-data channels are independent. Capture
    // either channel until both halves of one transaction are available.
    ...
    // All implemented configuration bits occupy byte lane zero.
    assign next_config = write_strb[0] ? write_data[4:0] : config_reg;
